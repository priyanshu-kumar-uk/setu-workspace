...
export const sendOtpService = async (email) => {
    ...
    if (exitUser && exitUser.otpExpiresAt && exitUser.otpExpiresAt > Date.now()) {  // for expire otp time
    ...
    const otpExpires = Date.now() + 3 * 60 * 1000  // 7 mints

    const user = await userModel.findOneAndUpdate(   //  solve the problem of  update same documet and new create if email same and not verifed
        { email: email },
        {
            otpCode: otp,
            otpExpiresAt: otpExpires,
            isVerified: false,
            email: email            // email add from in state
        }, {
        upsert: true,   // Agar email nahi mila, toh naya user CREATE kar do
        new: true,      // Update/Create hone ke baad naya wala document return karo
        setDefaultsOnInsert: true // Schema ke default values (jaise expireAt) apply karo
    })

    const mailoptions = {
        from: `"Setu Security" <${config.APP_EMAIL}>`, // Thoda aur secure sounding sender name
        to: email,
        subject: "Setu: Verification Code", // Clean aur direct subject line
        html: `
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
export const verifyOtpService = async (email, otp) => {
    ...
    user.otpCode = undefined      // delete a automatic  by
    user.otpExpiresAt = undefined
    user.expireAt = new Date(Date.now() + 10 * 60 * 1000) // Date wrap becuse it convert ka time into mongo DateObject
    await user.save()
...
export const loginService = async (email, password) => {
    ...
    const accessToken = jwt.sign(   // React will have to store in memory
        {
            id: user._id
        }, config.TOKEN,
        {
    ...
    user.password = undefined  // becuse this password should not go forntend
