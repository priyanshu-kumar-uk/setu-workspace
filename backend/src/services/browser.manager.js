/**
 * browser.manager.js
 * In-memory manager for collaborative browser sessions.
 * Model: [ 1 Room = 1 Browser Instance ]
 *
 * Each room gets one Playwright browser, one BrowserContext,
 * and a Map of tabId → Page. A screenshot loop streams JPEG
 * frames of the active tab to all connected sockets.
 */
...
/**
 * Formats raw user input into a navigable URL.
 * - If it looks like a URL (contains dot + no spaces), prepend https://
...
async function buildTabsState(session) {
  ...
  for (const [id, page] of session.tabsMap) {
    let title = '';
    try { title = await page.title(); } catch { /* page may be closed */ }
...
function startScreenshotLoop(roomId, session, io) {
  if (session.frameIntervalId) return; // already running
  ...
  session.frameIntervalId = setInterval(async () => {
    try {
      ...
      // Silently ignore screenshot failures (page navigating, etc.)
    }
  }, FRAME_INTERVAL_MS);
}
...
/**
 * Get or create a browser session for a room.
 * Launches Chromium, creates a context, opens one initial tab.
 */
...
/**
 * Create a new tab in the room's browser context.
 */
...
/**
 * Close a tab and switch to the nearest available one.
 */
...
/**
 * Go back in the active page history.
 */
...
/**
 * Go forward in the active page history.
 */
...
/**
 * Route a user interaction to the correct Playwright page.
 * Supported types: click, mousemove, scroll, keydown, keyup
 */
...
/**
 * Register a socket user in the room session.
 */
...
/**
 * Remove a socket user. If room is empty, destroy the session.
 */
...
/**
 * Get the current tabs state for a room (for sync on join).
 */
